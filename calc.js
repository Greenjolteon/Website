//Also add thing similar to the fractions, such as if the solution is √7, return √7, not 2.6457513111


function numapp(item) {document.form.input.value += item;}

function reset() {document.form.input.value = "";}
...
function equate() {
	...
	for (let i = 1; i < array.length; i++) {
		if ((constCheck(array[i]) !== array[i]) && isDigit(array[i - 1])) {
			array.splice(i, 0, '*');
		} else if (i >= 2 && array[i-1] == '(' && isDigit(array[i-2])) {
			array.splice(i-1, 0, '*');
		}
	}
	...
	//other constants can be put here
		...
		for (let q = 2; q < 500; q++) {
			if (Math.abs(array[0] - (1 / q)) < (10 ** -10)) {
				array[0] = '1/' + q + ' ≈ ' + 1 / q;
				return 0;
			} else if (Math.abs(array[0] - (Math.sqrt(q))) < (10 ** -10) && !isDigit(Math.sqrt(q))) {
				array[0] = '√' + q + ' ≈ ' + Math.sqrt(q);
				return 0;
			}
			for (let p = 2; p < q; p++) {
